...
class TestInterview:
    ...
    def test_interview_manager_can_load_and_update_messages_from_local_file(
        self, monkeypatch
    ):
        ...
        data = interview_manager.load_data(interview_manager.user_id)
        data.progress = 1
        data.messages.append(ChatMessage(role="user", content="Sure, I'd be happy to."))
        interview_manager.save_data(data)

        loaded_data = interview_manager.load_data(interview_manager.user_id)
        assert loaded_data.progress == 1
        assert loaded_data.messages[0] == ChatMessage(
            role="user", content="Sure, I'd be happy to."
        )
    ...
    def test_interview_manager_can_load_and_update_messages_from_minio(
        self, monkeypatch
    ):
        ...
        initial_data = InterviewData(
            user_id=interview_manager.user_id,
            progress=-1,
            history=[],
            messages=[],
