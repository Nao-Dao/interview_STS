...
# 脑岛平台的访谈
import os, json, time
from ..utils.cache import cache as CacheUtils
...
"""
    ...
    def __init__(self, id: int):
        ...
        self.last_judge_time = self.last_activate_time
    ...
    def judge(self, chat: Callable[
        ...
        判断议题是否结束
        """
        if self.last_judge_time == self.last_activate_time or \
            self.data.progress < 0:
            # 在最后一次检测之前，记录没有更改，无需检测
            return
        self.last_judge_time = self.last_activate_time
    ...
    def save_audio(self, audio: bytes):
        path = CacheUtils.get_path(CacheUtils.save(audio, "wav"))
        self.data.history[-1].audio_path.append(path)
        self.save_data(self.data)
