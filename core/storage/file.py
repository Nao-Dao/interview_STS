...
class FileStorage(InterviewStorage):
    ...
    def save(self, data: InterviewData) -> bool:
        try:
            ...
            json_path = os.path.join(
                self.base_path, str(data.user_id), "interview.json"
            )
    ...
    def load(self, user_id: str) -> Optional[InterviewData]:
        try:
            json_path = os.path.join(self.base_path, str(user_id), "interview.json")
    ...
    def exists(self, user_id: str) -> bool:
        json_path = os.path.join(self.base_path, user_id, "interview.json")
        return os.path.exists(json_path)

    def save_audio(self, user_id, data, format: str = None) -> str:
        audio_id = generate_snowflake_id()
        interview_audio_path = os.path.join(self.base_path, user_id, "audios")
    ...
    def _get_audio_file_path(
        self, user_id_path, audio_id: int, format: str = None
    ) -> str:
        matches = list(Path(user_id_path).glob(f"{audio_id}*"))
        if len(matches) > 0:
            base_path = matches[0]
        else:
            base_path = os.path.join(user_id_path, str(audio_id))
